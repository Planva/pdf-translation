...
import { cookies } from "next/headers"; // ✅ 只保留这一处
import {
  createKVSession,
  deleteKVSession,
  getKVSession,
...
export async function setSessionTokenCookie({
  ...
  userId,
  expiresAt,
}: { token: string; userId: string; expiresAt: Date }): Promise<void> {
  const jar = await cookies(); // ✅ 使用上面唯一的一处 import

  // 关键点：
  // - 生产用 SameSite: "none" + secure: true，保证 Stripe 之类的跨站（含 POST）回跳也会带上 cookie
  // - 开发环境保持 lax，避免 http 本地环境下浏览器拒收 secure+none
  jar.set(SESSION_COOKIE_NAME, `${userId}:${token}`, {
    httpOnly: true,
    secure: isProd,
    sameSite: isProd ? "none" : "lax",
    path: "/",
    expires: expiresAt,
    // 不要设置 domain，保持 HostOnly，避免 apex/www 不一致时丢 cookie
  });
}
